/*   Remote Controlled Binary Counter using the IR remote library
 ...
 This sketch increases a 2 bit number every time the channel up key on a remote is pressed
 and decreases a 2 bit number every time the channel down key on a remote is pressed
 and shows the output on th built in lED's  
...
{  
  if (irrecv.decode(&results))   // check whether a signal was received and decoded
  {
    ...
    digitalWrite(32,b);  // write middle bit  
    digitalWrite(33,c);  // write LSB  
